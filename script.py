...
warnings.simplefilter(action='ignore', category=FutureWarning)  # 忽略FutureWarning型warning
...
def application_train_test(num_rows=None, nan_as_category=True):
    ...
    for bin_feature in ['CODE_GENDER', 'FLAG_OWN_CAR', 'FLAG_OWN_REALTY']:
        df[bin_feature], uniques = pd.factorize(df[bin_feature])  # 将这三个feature变成数字0，1
    df, cat_cols = one_hot_encoder(df, nan_as_category)  # one-hot编码
